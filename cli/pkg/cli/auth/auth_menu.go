...
const (
    ...
    AuthActionNTdoTuringCoderLogin     AuthAction = "nt_account_login"
...
func RunAuthFlow(ctx context.Context, args []string) error {
    // 初始化认证状态（自动重新登录）
    if err := InitializeTuringCoderAuth(ctx); err != nil {
...
func HandleAuthMenuNoArgs(ctx context.Context) error {
    ...
    case AuthActionNTdoTuringCoderLogin:
        return HandleTuringCoderNTAuth(ctx)
    case AuthActionPlatformLogin:
        return HandleTuringCoderPlatformAuth(ctx)
    case AuthActionExit:
        return nil
    default:
...
func ShowAuthMenuWithStatus(isClineAuthenticated bool, hasOrganizations bool, currentProvider, currentModel string) (AuthAction, error) {
    ...
    if isClineAuthenticated {
        ...
        options = []huh.Option[AuthAction]{
            ...
            huh.NewOption("Authenticate with NT account", AuthActionNTdoTuringCoderLogin),
